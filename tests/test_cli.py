...
class TestCLI(TenTestCase):
    ...
    def test_transform_qs_to_json(self) -> None:
        _, output = self._test_program(transform, "qs.parse", "json.encode", input=b"a=3&b=2")
        self.assertEqual(output, b'{"a": "3", "b": "2"}\n')
        
    def test_transform_wrong_transform(self) -> None:
        output, _ = self._test_program(transform, "qs.doesnotexist", "json.encode", input=b"a=3&b=2")
        self.assertIn("Unknown transform qs.doesnotexist\n", output)
        
    def test_transform_wrong_module(self) -> None:
        output, _ = self._test_program(transform, "doesnotexist.help", "json.encode", input=b"a=3&b=2")
    ...
    def test_transform_with_python(self) -> None:
        _, output = self._test_program(transform, "base64.decode", "--python", input=b"AAAA")
